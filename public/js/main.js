...
const user=JSON.parse(localStorage.getItem('targetUser'))
...
if (currentTime < 12) {
  heroHeading.innerHTML = `Good Morning, <span>${name}</span> <span class="emoji">☀️</span>! `} 
  else if (currentTime < 18) {
  heroHeading.innerHTML = `Good Afternoon, <span>${name}</span> <span class="emoji">☀️</span>! `}
 else {
  heroHeading.innerHTML =`Good Evening, <span>${name}</span> <span class="emoji">☀️</span>! `
}
